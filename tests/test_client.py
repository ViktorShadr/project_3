...
def test_check_connection_success(api_client):
    with patch("requests.get") as mock_get:
        ...
        mock_get.assert_called_once_with(f"{BASE_URL}/vacancies", params={"per_page": 1}, timeout=5)
...
def test_get_success(api_client):
    ...
    with patch("requests.get") as mock_get:
        ...
        assert result == data
        mock_get.assert_called_once_with(f"{BASE_URL}/test_endpoint", params=None, timeout=10)
...
def test_get_employer(api_client):
    with patch.object(ApiClient, "get", return_value={"id": COMPANY_ID, "name": "Test Co"}):
